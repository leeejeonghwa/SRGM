...
for i in range(1, len(X_test)):
    lambda_ti = a_optimized * b_optimized **2 * i * np.exp(-b_optimized*i)   # 현재 시간 스텝에서 모델의 예측값
    lambda_ti_minus_1 = a_optimized * b_optimized **2 * (i-1) * np.exp(-b_optimized*(i-1)) # 이전 시간 스텝에서 모델의 예측값
...
M10_results_list = [round(bias, 3), round(mse, 3), round(meop, 3), round(mae, 3), round(noise, 3), round(prr, 3), round(variance, 3), round(rsq, 3), round(ts, 3)]
print(M10_results_list)
