...
def logistic_Growth_model_cumulative(t, a, b, k):
    return a / (1 + k * np.exp(-b * t))
...
popt, pcov = curve_fit(logistic_Growth_model_cumulative, X_train,y_train ,method = 'trf')

a_optimized, b_optimized,k_optimized=popt

# Generate predictions using the optimized parameters for both training and testing data
failures_predictions_train = logistic_Growth_model_cumulative(X_train, a_optimized, b_optimized, k_optimized)
failures_predictions_test = logistic_Growth_model_cumulative(X_test, a_optimized, b_optimized, k_optimized)
...
for i in range(1, len(X_test)):
    lambda_ti = a_optimized * b_optimized * k_optimized * np.exp(-b_optimized * i) / (1 + k_optimized * np.exp(-b_optimized * i) ** 2)   # 현재 시간 스텝에서 모델의 예측값
    lambda_ti_minus_1 = a_optimized * b_optimized * k_optimized * np.exp(-b_optimized * i - 1) / (1 + k_optimized * np.exp(-b_optimized * i - 1) ** 2)   # 이전 시간 스텝에서 모델의 예측값
...
print(f"Optimized Parameters - a: {a_optimized}, b: {b_optimized}, k:{k_optimized}")
print("-------------------------------------------------------------------")
...
M5_results_list = [round(bias, 3), round(mse, 3),round(meop, 3),round(mae, 3),round(noise, 3),round(prr, 3),round(variance, 3),round(rsq, 3),round(ts, 3)]
print(M5_results_list)