...
def yamada_imperfect_debugging2_model(t, a, b, alpha):
    return a * (1 - np.exp(-b * t)) * (1 - (alpha / b)) + alpha * a * t
...
failures_predictions_train = yamada_imperfect_debugging2_model(X_train, a_optimized, b_optimized, alpha_optimized)
failures_predictions_test = yamada_imperfect_debugging2_model(X_test, a_optimized, b_optimized, alpha_optimized)
...
for i in range(1, len(X_test)):
    lambda_ti = a_optimized*b_optimized*np.exp(-b_optimized*i)*(1-alpha_optimized/b_optimized) + alpha_optimized*a_optimized  # 현재 시간 스텝에서 모델의 예측값
    lambda_ti_minus_1 = a_optimized*b_optimized*np.exp(-b_optimized*(i-1))*(1-alpha_optimized/b_optimized) + alpha_optimized*a_optimized  # 이전 시간 스텝에서 모델의 예측값
...
print(f"Optimized Parameters - a: {a_optimized}, b: {b_optimized}, c: {alpha_optimized}")
print("-------------------------------------------------------------------")
...
M11_results_list = [round(bias, 3), round(mse, 3), round(meop, 3), round(mae, 3), round(noise, 3), round(prr, 3), round(variance, 3), round(rsq, 3), round(ts, 3)]
print(M11_results_list)
