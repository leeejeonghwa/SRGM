...
def Gompertz_model_cumulative(t, a, b, k):
    return a * k * np.exp (-b * t)
...
popt, pcov = curve_fit(Gompertz_model_cumulative, X_train,y_train ,method = 'trf')
...
failures_predictions_train = Gompertz_model_cumulative(X_train, a_optimized, b_optimized, k_optimized)
failures_predictions_test = Gompertz_model_cumulative(X_test, a_optimized, b_optimized, k_optimized)
...
for i in range(1, len(X_test)):
    lambda_ti = a_optimized * b_optimized * np.log(k_optimized) * k_optimized ** np.exp(-b_optimized * i) * np.exp(-b_optimized * i) # 현재 시간 스텝에서 모델의 예측값
    lambda_ti_minus_1 = a_optimized * b_optimized * np.log(k_optimized) * k_optimized ** np.exp(-b_optimized * i - 1) * np.exp(-b_optimized * i - 1)   # 이전 시간 스텝에서 모델의 예측값
...
M3_results_list = [round(bias, 3), round(mse, 3),round(meop, 3),round(mae, 3),round(noise, 3),round(prr, 3),round(variance, 3),round(rsq, 3),round(ts, 3)]
print(M3_results_list)