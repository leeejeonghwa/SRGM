...
# Define the Musa-Okumoto model function for cumulative data
def musa_okumoto_model(t, a, b):
...
popt, pcov = curve_fit(musa_okumoto_model, X_train, y_train,method = 'trf', bounds=([0, 0], [np.inf, 1]))

# Extract optimized parameters
a_optimized, b_optimized = popt

# Generate predictions using the optimized parameters for both training and testing data
failures_predictions_train = musa_okumoto_model(X_train, a_optimized, b_optimized)
failures_predictions_test = musa_okumoto_model(X_test, a_optimized, b_optimized)
...
for i in range(1, len(X_test)):
    lambda_ti = a_optimized * b_optimized/ (1+b_optimized*i) # 현재 시간 스텝에서 모델의 예측값
    lambda_ti_minus_1 =  a_optimized * b_optimized/ (1+b_optimized*i-1)# 이전 시간 스텝에서 모델의 예측값
...
M7_results_list = [round(bias, 3), round(mse, 3),round(meop, 3),round(mae, 3),round(noise, 3),round(prr, 3),round(variance, 3),round(rsq, 3),round(ts, 3)]
print(M7_results_list)