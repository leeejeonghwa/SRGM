...
def yamada_imperfect_debugging_model1_cumulative(t, a, b, alpha):
    return (a * b * (np.exp(alpha * t) - np.exp(-b * t))) / (a + b)
...
popt, pcov = curve_fit(yamada_imperfect_debugging_model1_cumulative, X_train, y_train, method ='trf', max_nfev=30000)

# Extract optimized parameters
a_optimized, b_optimized, alpha_optimized= popt

# Generate predictions using the optimized parameters for both training and testing data
failures_predictions_train = yamada_imperfect_debugging_model1_cumulative(X_train, a_optimized, b_optimized, alpha_optimized)
failures_predictions_test = yamada_imperfect_debugging_model1_cumulative(X_test, a_optimized, b_optimized, alpha_optimized)
...
for i in range(1, len(X_test)):
    lambda_ti = a_optimized * b_optimized *(alpha_optimized*np.exp(alpha_optimized * i)+np.exp(-b_optimized*i))/alpha_optimized+b_optimized  # 현재 시간 스텝에서 모델의 예측값
    lambda_ti_minus_1 = a_optimized * b_optimized *(alpha_optimized*np.exp(alpha_optimized * i-1)+np.exp(-b_optimized*i-1))/alpha_optimized+b_optimized  # 이전 시간 스텝에서 모델의 예측값
...
print(f"Optimized Parameters - a: {a_optimized}, b: {b_optimized}, alpha:{alpha_optimized}")
print("-------------------------------------------------------------------")
...
M8_results_list = [round(bias, 3), round(mse, 3),round(meop, 3),round(mae, 3),round(noise, 3),round(prr, 3),round(variance, 3),round(rsq, 3),round(ts, 3)]
print(M8_results_list)