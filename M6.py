...
popt, pcov = curve_fit(modified_duane_model, X_train, y_train, method = 'trf', bounds=([0, 0, 0], [np.inf, 1 ,1]), max_nfev=30000)
...
for i in range(1, len(X_test)):
    lambda_ti = a_optimized * c_optimized * b_optimized ** c_optimized * (b_optimized + i) ** (1-c_optimized) # 현재 시간 스텝에서 모델의 예측값
    lambda_ti_minus_1 = a_optimized * c_optimized * b_optimized ** c_optimized * (b_optimized + i-1) ** (1-c_optimized)  # 이전 시간 스텝에서 모델의 예측값
...
M6_results_list = [round(bias, 3), round(mse, 3),round(meop, 3),round(mae, 3),round(noise, 3),round(prr, 3),round(variance, 3),round(rsq, 3),round(ts, 3)]
print(M6_results_list)